...
namespace TokenMen
{
    internal class Acl
    {
        internal static bool ChangeDesktopACL()
        {
            ...
            if (!PrivilegeEnabler(SE_SECURITY_NAME))
                return false;
        ...
        private static bool DekstopACLtoAll(IntPtr hObject, string objName)
        {
            ...
            if (!status)
            {
                Console.WriteLine("[-] Failed to call CreateWellKnownSid. Error String: {0}, Error Code: {1}, realsize: {2}, status: {3}", GetLastErrorString, GetLastErrorCode, realsize, status);
            ...
            if (retval != 0)
            {
                Console.WriteLine("[-] Failed to call SetEntriesInAcl on {2}. Error Code: {0} ({1})", GetLastErrorCode, GetLastErrorString, objName);
            }
